...
def preveri_cas_uporabe(cas_uporabe):
    if(isinstance(cas_uporabe, int)):
        if(cas_uporabe > 0 and cas_uporabe <= MAKSIMALEN_ROK):
            ...
            raise ValueError(
                "Cas uporabe je negativen, nič ali pa presega maksimalen rok.")
    else:
...
# Potrebno je previdno izbiranje krajših ključnih besed, saj obstaja
# možnost napačnega identificiranja živil iz niza (iz računa).
...
class Zivilo:
    ...
    def __init__(
        ...
        # Posamezna zivila nekega tipa imajo lahko različen cas uporabe kot tip
        # katerega so.
    ...
    def je_to_zivilo(self, niz):
        if isinstance(niz, str):
            ...
            raise ValueError(
                "Obdelovan podatek ni niz pri izvajanju funkcije je_to_zivilo.")
...
# Nakup_zivila hrani maso in datum nakupa. Na podlagi tipa zivila izracuna
# se rok uporabe. Omogoča pretvorbo v slovar(in nazaj).
...
class Nakup_zivila:
    def __init__(self, zivilo, masa=None, datum_nakupa=None, datum_roka=None):
        ...
        # dodaj datum roka in datuma nakupa ce sta definirana drugače vzemi
        # današnji datum za datum nakupa in mu prištej rok uporabe za datum
        # roka
        ...
        if datum_roka is None:
            self.datum_roka = self.datum_nakupa + \
                datetime.timedelta(days=self.zivilo.cas_uporabe)
        else:
    ...
    def iz_slovarja(slovar):
        return Nakup_zivila(
            Zivilo.iz_slovarja(
                slovar["zivilo"]), slovar["masa"], datetime.date.fromisoformat(
                slovar["datum_nakupa"]), datetime.date.fromisoformat(
                slovar["datum_roka"]))
    ...
    def nakup_zivila_iz_niza(cls, niz):
        # gre čez vsa živila v podatkovni bazi in preveri ali so vse ključne
        # besede vsebovane v nizu
        for zivilo in Zivilo.zivila:
...
############################################################
# Hrani seznam nakupljenih živil  in pa kdaj se je ta nakup zgodil
# (predvideva kar trenuten datum). Omogoča pretvorbo v slovar (in nazaj).
...
class Nakup:
    ...
    def v_slovar(self):
        ...
        return {
            ...
            # datum pretvorimo v isoformat za shranjevanje v besedilo
